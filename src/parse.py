...
def parse(xlsx: str) -> ParsedData:
    ...
    if data_age_val:
        if isinstance(data_age_val, str):
            ...
            data_age = data_age_val.strftime('%d.%m.%Y') # Convert datetime to string if it's a datetime object

    people = []

    for row in ws.iter_rows(min_row=5, max_col=12):
        _, status, _, name, vorname, untersuchung_val, \
            unterweisung_val, anlage_val, agtUebung_val, _, _, _ = (c.value for c in row)
